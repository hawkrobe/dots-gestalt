...
tester <- function(itemrange, left, right) {
  for (cnd in c("Orientation_NO_Lines", "Orientation_Lines") ) {
    for (i in itemrange) {
...
allData <- read.csv("~/Box Sync/Dots/Analysis/Experiment2_data.txt")

# # Remove all participants that did not complete all trials
 allData <- subset(allData, allData$Subject != "KBL" & allData$Subject != "ZL" &
                            allData$Subject != "GAM" & allData$Subject != "LNB"&
                            allData$Subject != "CCW")
...
ControlTrials <- with(allData, Stim1!="None" & Stim1 == Stim2)
ControlLeft  <- with(allData, Stim1!="None" & Stim1 !="Absent" & Stim2=="Absent")
ControlRight <- with(allData, Stim2!="None" & Stim2 !="Absent" & Stim1=="Absent")

ConfiguralTrials <- with(allData, Stim1!="None" & Stim1!="Absent" & Stim2!="None" & Stim2 != "Absent" & !ControlTrials)
ConfiguralLeft   <- with(allData, (Stim1=="DownRight" | Stim1=="UpRight") & Stim2=="Absent")
ConfiguralRight  <- with(allData, (Stim2=="DownLeft"  | Stim2=="UpLeft")  & Stim1=="Absent")
...
sftDataControl    <- sftData[ControlTrials | ControlLeft | ControlRight,]
sftDataConfigural <- sftData[ConfiguralTrials | ConfiguralLeft | ConfiguralRight,]
sftData_configural_lines <- subset(sftDataConfigural, sftDataConfigural$Condition == "Orientation_Lines")
sftData_configural_nolines <- subset(sftDataConfigural, sftDataConfigural$Condition == "Orientation_NO_Lines")
sftData_control_lines <- subset(sftDataControl, sftDataControl$Condition == "Orientation_Lines")
sftData_control_nolines <- subset(sftDataControl, sftDataControl$Condition == "Orientation_NO_Lines")

sftData_singleDot <- with(sftData, 
                          subset(sftData, (Channel1 == 0 & Channel2 == 1)
                                 | (Channel1 == 1 & Channel2 == 0)))

capControl <- capacityGroup(sftDataControl, stopping.rule="OR", acc.cutoff=.6, plotCt=F)
capConfigural <- capacityGroup(sftDataConfigural, stopping.rule="OR", acc.cutoff=.6, plotCt=F)
...
for (i in 1:length(subject_codes)) { accuracies[i] = with(sftData_configural_nolines, mean(Correct[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0]))}
for (i in 1:length(subject_codes)) { accuracies[nsubjects+i] = with(sftData_configural_lines, mean(Correct[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0]))}
for (i in 1:length(subject_codes)) { accuracies[2*nsubjects+i] = with(sftData_control_nolines, mean(Correct[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0]))}
for (i in 1:length(subject_codes)) { accuracies[3*nsubjects+i] = with(sftData_control_lines, mean(Correct[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0]))}
print(accuracies)

# Compute RTs on CORRECT Configural trials
RTs <- rep(0, length(subject_codes)*4)
for (i in 1:length(subject_codes)) { RTs[i] = with(sftData_configural_nolines, mean(RT[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0 & Correct == 1]))}
for (i in 1:length(subject_codes)) { RTs[nsubjects+i] = with(sftData_configural_lines, mean(RT[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0 & Correct == 1]))}
for (i in 1:length(subject_codes)) { RTs[2*nsubjects+i] = with(sftData_control_nolines, mean(RT[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0 & Correct == 1]))}
for (i in 1:length(subject_codes)) { RTs[3*nsubjects+i] = with(sftData_control_lines, mean(RT[Subject == subject_codes[i] & Channel1 > 0 & Channel2 > 0 & Correct == 1]))}
print(RTs)
table2 <- data.frame(P=1:nsubjects)
for ( i in 1:nsubjects ) {
  table2$b111[i] <- "&"
  table2$Zfig_l[i] <- round(CzMat$Cz[i],2)
  table2$b[i] <- "&"
  table2$Accfig_l[i] <- round(accuracies[i], 2)
  table2$b2[i] <- "&"
  table2$RTfig_l[i] <- round(RTs[i])
  table2$b3[i] <- "&"
  table2$Zfig_no_l[i] <- round(CzMat$Cz[nsubjects+i],2)
  table2$b4[i] <- "&"
  table2$Accfig_no_l[i] <- round(accuracies[nsubjects+i], 2)
  table2$b5[i] <- "&"
  table2$RTfig_no_l[i] <- round(RTs[nsubjects+i])
  table2$b6[i] <- "&"
  table2$Ztrol_l[i] <- round(CzMat$Cz[2*nsubjects+i],2)
  table2$b7[i] <- "&"
  table2$Acctrol_l[i] <- round(accuracies[2*nsubjects+i], 2)
  table2$b8[i] <- "&"
  table2$RTtrol_l[i] <- round(RTs[2*nsubjects+i])
  table2$b9[i] <- "&"
  table2$Ztrol_no_l[i] <- round(CzMat$Cz[3*nsubjects+i],2)
  table2$b10[i] <- "&"
  table2$Acctrol_no_l[i] <- round(accuracies[3*nsubjects+i], 2)
  table2$b11[i] <- "&"
  table2$RTtrol_no_l[i] <- round(RTs[3*nsubjects+i])
  table2$b12[i] <- "&"
  table2$Acc_single[i] <- mean(with(sftData_singleDot, Correct[Subject == subject_codes[i]]))
  table2$b13[i] <- "&"
  table2$RT_single[i] <- mean(with(sftData_singleDot, RT[Subject == subject_codes[i] & Correct == 1]))
}
...
allData$Configural[ConfiguralTrials] <- "Configural"
allData$Configural[ControlTrials] <- "Control"
allData$Configural[allData$Stim1=="Absent" | allData$Stim2=="Absent"] <- "SingleDistractor"
allData$Configural[ConfiguralLeft | ConfiguralRight | ControlLeft | ControlRight] <- "SingleTarget"
allData$Configural[allData$Stim1=="None" & allData$Stim2=="None"] <- "Distractor"
allData$Configural <- as.factor(allData$Configural)

allData$Lines[allData$Condition=="Orientation_NO_Lines"] <- "NoLines"
allData$Lines[allData$Condition=="Orientation_Lines"] <- "Lines"
...
postscript("Box Sync/Dots/VisionResearch_Paper/images/Exp2_Results.eps", width=7.40, height=2.5,
